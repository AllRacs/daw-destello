...
function filtrado(){
    ...
    if(!isset($_POST['input_pass_modify'])){// Si viene de registro
        ...
        }else if(preg_match('`[ÑñçÇ]`',$_POST['input_pass'])){

            $result = false;
            echo 'Las contraseñas deben contener solo caracteres ingleses';
        ...
        if(preg_match('`[ÑñçÇ]`',$_POST['input_pass_modify'])){

            $result = false;
            echo 'Las contraseñas deben contener solo caracteres ingleses';

        }else if(!preg_match('`[a-z]`',$_POST['input_pass_modify'])){ //Comprobar que contiene letra minuscula
